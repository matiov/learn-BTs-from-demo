...
def generate_launch_description():
    ...
    spatial_share_dir = get_package_share_directory('spatial_ros')
    spatial_paths = FilePaths()
    # Process templated node parameters file (i.a. setting correct root namespace).
    try:
        spatial_paths.node_parameters = process_templated_file(
            os.path.join(spatial_share_dir, 'config', 'spatial.yaml'),
            rosified_hostname
        )
    except Exception as exception:
        print('Failed to process templated file: {0}'.format(str(exception)))
    ...
    ld.add_action(
        Node(
            ...
            namespace=namespaces.get_root_robot(),
            remappings=[
                ('~/plan_next_move', 'plan_next_move'),
                ('~/set_specification', 'set_specification'),
                ('~/set_unmovable', 'set_unmovable'),
                ('tf', '/tf'),
                ('tf_static', '/tf_static'),
            ],
            parameters=[spatial_paths.node_parameters],
            output='screen',
            emulate_tty=True,
        )
    )
